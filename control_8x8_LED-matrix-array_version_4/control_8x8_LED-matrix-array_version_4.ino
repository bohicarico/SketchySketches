...
//else if (checkflag == 1 or checkflag == 2){

// turn all the rows on, only column is being driven dynamically
digitalWrite(R1,LOW); 
digitalWrite(R2,LOW); 
digitalWrite(R3,LOW); 
digitalWrite(R4,LOW); 
...
//rows(0);    // by row L-to-R top-to-bottom R-to-L bottom to top
//columns(0); // by column R-to-L top-to-bottom then L-to-R bottom to top
//columns(1); // vertical columns left-to-right and vice versa ( need to tweak delay )
columns(2);   // horizontal columns left-to-right
}
